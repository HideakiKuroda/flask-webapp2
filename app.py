...
def list_files(folder_id=None):
    files, folders, error, status = ms_file_control.list_files(auth, app_config, folder_id)
    if error:
        return error, status
...
def download_file(file_id, file_name):
    file_content, error, status = ms_file_control.download_file(file_id, auth, app_config)
    if error:
        return error, status
...
def create_folder():
    ...
    folder_id, error = ms_file_control.create_folder(folder_name, auth, app_config)
    if error:
        flash(error)
        return redirect(url_for("index"))
